"""
MYM Opening Range Breakout — NY Session 1-MIN — 6 Contracts: TP1(2), TP2(2), Runner(2)
========================================================================================
  - Opening range: NY 9:30-9:44 (America/New_York)
  - Breakout entry on close above/below range after 9:45
  - 6 contracts split 2/2/2: TP1=43pts, TP2=45pts, Runner trail=55pts
  - BE trigger=45pts, Max stop=180pts, Range width: 115-205pts
  - Volume filter: SMA(20), mult 0.5
  - ATR filter: length 12, avg lookback 40, min mult 1.0, max mult 2.0
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    # ── Unpack parameters ────────────────────────────────────────────────────
    tp1_pts       = float(params.get("first_tp_points", 43.0))
    tp2_pts       = float(params.get("second_tp_points", 45.0))
    trail_dist    = float(params.get("trail_distance_points", 55.0))
    runner_be_pts = float(params.get("runner_be_trigger_points", 45.0))
    contracts     = int(params.get("contracts_per_trade", 6))
    c1            = int(params.get("tp1_contracts", 2))
    c2            = int(params.get("tp2_contracts", 2))
    max_stop_pts  = float(params.get("max_stop_points", 180.0))
    min_range_w   = float(params.get("min_range_width", 115.0))
    max_range_w   = float(params.get("max_range_width", 205.0))
