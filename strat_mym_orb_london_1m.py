"""
MYM Opening Range Breakout — London Session 1-MIN — 6 Contracts: TP1(2), TP2(2), Runner(2)
============================================================================================
  - Opening range: London 8:00-8:14 (Europe/London)
  - Breakout entry on close above/below range after 8:15
  - 6 contracts split 2/2/2: TP1=25pts, TP2=30pts, Runner trail=20pts
  - BE trigger=30pts, Max stop=77pts, Range width: 60-115pts
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    ...
    trail_dist    = float(params.get("trail_distance_points", 20.0))
    runner_be_pts = float(params.get("runner_be_trigger_points", 30.0))
    contracts     = int(params.get("contracts_per_trade", 6))
    c1            = int(params.get("tp1_contracts", 2))
    c2            = int(params.get("tp2_contracts", 2))
    max_stop_pts  = float(params.get("max_stop_points", 77.0))
    min_range_w   = float(params.get("min_range_width", 60.0))
    max_range_w   = float(params.get("max_range_width", 115.0))
...
def _trade(entry_t, exit_t, entry_px, exit_px, direction, contracts, pnl, reason):
    return {
        ...
        "pnl":         pnl,
        "exit_reason": reason,
        "entry_type":  "ORB London 1min MYM",
