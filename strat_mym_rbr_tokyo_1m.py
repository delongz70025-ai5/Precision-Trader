"""
MYM Tokyo Rally-Base-Rally (1min) — 6 Contracts: TP1(2), TP2(2), Runner(2)
============================================================================
  - Tokyo session 09:00-11:00 JST
  - EMA 5/20 trend filter
  - Volume spike filter (bar vol >= multiplier * SMA)
  - Rally-Base-Rally (bullish) / Drop-Base-Drop (bearish) pattern
  - EMA touch confirmation
  - 6 contracts split 2/2/2: TP1 (1.2x risk), TP2 (avg of first_rr and runner_rr), Runner (3.1x risk)
  - Fixed risk stop-loss at 14 pts
"""
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    # ── Unpack parameters ────────────────────────────────────────────────────
    fast_len          = int(params.get("fast_len", 5))
    slow_len          = int(params.get("slow_len", 20))

    vol_lookback      = int(params.get("vol_lookback", 20))
    vol_multiplier    = float(params.get("vol_multiplier", 2.3))
    ...
    fixed_risk_points = float(params.get("fixed_risk_points", 14.0))
    first_rr_ratio    = float(params.get("first_rr_ratio", 1.2))
    runner_rr_ratio   = float(params.get("runner_rr_ratio", 3.1))
    tp2_rr_ratio      = float(params.get("tp2_rr_ratio", (1.2 + 3.1) / 2.0))
    ...
    if _idx.tz is None:
        _tk_index = _idx.tz_localize("UTC").tz_convert(tokyo_tz)
    else:
        _tk_index = _idx.tz_convert(tokyo_tz)
    hours_arr = _tk_index.hour.values
    minutes_arr = _tk_index.minute.values
    ...
    for i in range(n):
        ...
        # Session: 09:00 - 11:00 JST
        in_session = (9 <= tk_hour < 11)
