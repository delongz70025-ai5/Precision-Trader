"""
MNQ Opening Range Breakout — Tokyo Session 1-MIN — 6 Contracts: TP1(2), TP2(2), Runner(2)
════════════════════════════════════════════════════════════════════════════════
  - Opening range: Tokyo 9:00-9:14 (Asia/Tokyo)
  - Breakout entry on close above/below range after 9:15
  - 6 contracts split 2/2/2: TP1=6pts, TP2=7pts, Runner trail=6pts
  - BE trigger=6pts, Max stop=59pts, Range width: 34-160pts
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    # ── Unpack parameters ────────────────────────────────────────────────────
    tp1_pts       = float(params.get("first_tp_points", 20.0))
    tp2_pts       = float(params.get("second_tp_points", 30.0))
    trail_dist    = float(params.get("trail_distance_points", 1.0))
    runner_be_pts = float(params.get("runner_be_trigger_points", 6.0))
    contracts     = int(params.get("contracts_per_trade", 6))
    c1            = int(params.get("tp1_contracts", 2))
    c2            = int(params.get("tp2_contracts", 2))
    max_stop_pts  = float(params.get("max_stop_points", 70.0))
    min_range_w   = float(params.get("min_range_width", 40.0))
    max_range_w   = float(params.get("max_range_width", 100.0))

    flat_by_hr    = int(params.get("flatten_hour", 15))   # Tokyo 15:00
    ...
    for i in range(n):
        ...
        # ── Opening range: Tokyo 9:00-9:14 ──────────────────────────────────
        in_or = (tk_hour == 9 and 0 <= tk_min <= 14)
        if in_or:
            range_high = h if np.isnan(range_high) else max(range_high, h)
            range_low  = l if np.isnan(range_low)  else min(range_low, l)

        after_range   = (tk_hour > 9) or (tk_hour == 9 and tk_min >= 15)
        entry_allowed = tk_hour < flat_by_hr
        should_flat   = tk_hour >= flat_by_hr
        ...
        if pos > 0 and direction == 1 and not should_flat:
            ...
            if not tp1_filled and h >= entry_px + tp1_pts:
                tp1_filled = True
                pnl = (tp1_pts) * c1 * pt_val - c1 * fee_per_contract
                equity += pnl
                pos -= c1
                trades.append(_trade(entry_time, _idx[i], entry_px, entry_px + tp1_pts, 1, c1, pnl, "TP1"))

            # TP2
            if pos > 0 and not tp2_filled and h >= entry_px + tp2_pts:
                tp2_filled = True
                pnl = (tp2_pts) * c2 * pt_val - c2 * fee_per_contract
                equity += pnl
                pos -= c2
                trades.append(_trade(entry_time, _idx[i], entry_px, entry_px + tp2_pts, 1, c2, pnl, "TP2"))
