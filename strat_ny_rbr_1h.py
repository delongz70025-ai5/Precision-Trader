"""
MNQ NY Rally-Base-Rally (1H) — 2 Contracts: TP1 + Runner
═════════════════════════════════════════════════════════════
Converted from PineScript. Exact same logic:
  - NY session 09:30-15:00 ET
  ...
  - 2 contracts: TP1 (1.5x risk), Runner (3.6x risk)
  - Fixed risk stop-loss at 120 pts
"""
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    ...
    fixed_risk_points = float(params.get("fixed_risk_points", 120.0))
    first_rr_ratio    = float(params.get("first_rr_ratio", 1.5))
    runner_rr_ratio   = float(params.get("runner_rr_ratio", 3.6))
    ...
    # ── Pre-compute timezone arrays (data is US/Eastern) ─────────────────────
    _idx = df.index
    # Convert to NY time once for all bars
    ...
    for i in range(n):
        ...
        # Session: 09:30 - 15:00 ET
        in_session = (ny_hour == 9 and ny_min >= 30) or (10 <= ny_hour < 15)
