"""
MYM NY Rally-Base-Rally (1min) — 8 Contracts: TP1(4) + Runner(4)
=================================================================
  - NY session 09:30-11:30 ET
  ...
  - 8 contracts: TP1 4ct (1.4x risk), Runner 4ct (3.4x risk)
  - Fixed risk stop-loss at 35 pts
"""
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    ...
    vol_lookback      = int(params.get("vol_lookback", 45))
    vol_multiplier    = float(params.get("vol_multiplier", 1.7))
    ...
    fixed_risk_points = float(params.get("fixed_risk_points", 35.0))
    first_rr_ratio    = float(params.get("first_rr_ratio", 1.4))
    runner_rr_ratio   = float(params.get("runner_rr_ratio", 3.4))
    contracts         = int(params.get("contracts_per_trade", 8))
    tp1_contracts     = int(params.get("tp1_contracts", 4))
    ...
    if _idx.tz is None:
        _ny_index = _idx.tz_localize("UTC").tz_convert(ny_tz)
    else:
        _ny_index = _idx.tz_convert(ny_tz)
    hours_arr = _ny_index.hour.values
    minutes_arr = _ny_index.minute.values
    ...
    for i in range(n):
        ...
        in_session = ((ny_hour == 9 and ny_min >= 30)
                      or ny_hour == 10
                      or (ny_hour == 11 and ny_min < 30))
