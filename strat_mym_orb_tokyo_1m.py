"""
MYM Opening Range Breakout — Tokyo Session 1-MIN — 6 Contracts: TP1(2), TP2(2), Runner(2)
===========================================================================================
  - Opening range: Tokyo 9:00-9:14 (Asia/Tokyo)
  - Breakout entry on close above/below range after 9:15
  - 6 contracts split 2/2/2: TP1=45pts, TP2=50pts, Runner trail=45pts
  - BE trigger=50pts, Max stop=70pts, Range width: 41-120pts
...
def run_backtest(df: pd.DataFrame, params: dict) -> dict:
    # ── Unpack parameters ────────────────────────────────────────────────────
    tp1_pts       = float(params.get("first_tp_points", 45.0))
    tp2_pts       = float(params.get("second_tp_points", 50.0))
    trail_dist    = float(params.get("trail_distance_points", 45.0))
    ...
    min_range_w   = float(params.get("min_range_width", 41.0))
